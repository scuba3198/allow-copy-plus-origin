(function() {
  ...
  (window as any)[marker] = { attachedListeners: [] };
  ...
  const getRegistry = (): MainRegistry => {
    return (window as any)[marker] as MainRegistry;
  };
   ...
   */
  const targetDomains: Set<string> = new Set([
  ...
  const initOverride = (): void => {
    ...
    const copyEvents: Set<string> = new Set(["copy", "cut", "paste", "selectstart", "contextmenu", "dragstart", "keydown"]);
    ...
    EventTarget.prototype.addEventListener = function(
      ...
      if (copyEvents.has(type)) {
        ...
        registry.attachedListeners.push({
          ...
          type: type,
          original: listener,
          wrapper: wrapper as EventListener,
          options: options
        });
        originalAdd.call(this, type, wrapper as EventListener, options);
      } else {
    ...
    EventTarget.prototype.removeEventListener = function(
      ...
      if (copyEvents.has(type)) {
        const found = registry.attachedListeners.find(
  ...
  try {
     ...
     */
    const captureBlocker = (e: Event): void => {
      if (document.body && document.body.classList.contains(hostClass)) {
