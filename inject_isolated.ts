...
(function() {
   ...
   */
  const clearDirectHandlers = (el: HTMLElement | Document): void => {
    eventProps.forEach(prop => {
      try {
        const key = prop as keyof (HTMLElement | Document);
        const val = el[key];
   ...
   * @param el - The DOM element or document.
   */
  const restoreDirectHandlers = (el: HTMLElement | Document): void => {
    try {
      const handlers = el[backupHandlersKey];
      if (handlers) {
        eventProps.forEach(prop => {
          if (handlers[prop] !== undefined) {
            (el as any)[prop] = handlers[prop];
          }
        });
   ...
   * @param el - The DOM element.
   */
  const restoreDraggables = (el: HTMLElement): void => {
    try {
   ...
   * @param el - The DOM element.
   */
  const fixStyling = (el: HTMLElement): void => {
      ...
      try {
        ...
        if (selectBlocked || pointerBlocked) {
          el[backupStylesKey] = {
            ...
            webkitUserSelect: (el.style as any).webkitUserSelect || "",
   ...
   * @param el - The DOM element.
   */
  const restoreStyling = (el: HTMLElement): void => {
    try {
      ...
      if (styles) {
        ...
        if (styles.webkitUserSelect) (el.style as any).webkitUserSelect = styles.webkitUserSelect;
   ...
   */
  const preventBypass = (e: Event): void => {
    if (e.type === "keydown" || e.type === "keyup") {
      const keyEvent = e as KeyboardEvent;
      if (!keyEvent.ctrlKey && !keyEvent.metaKey) return;
      if (keyEvent.key !== 'c' && keyEvent.key !== 'x' && keyEvent.key !== 'a') return;
    }
    e.stopPropagation();
   ...
   */
  const observeMutations = (target: Node): void => {
    try {
   ...
   */
  const messageListener = (msg: any, _sender: chrome.runtime.MessageSender, sendResponse?: (response?: any) => void): void => {
    ...
    document.querySelectorAll("*").forEach(el => {
      ...
      if (htmlEl.shadowRoot) {
        htmlEl.shadowRoot.querySelectorAll("*").forEach(subEl => {
          const subHtmlEl = subEl as HTMLElement;
